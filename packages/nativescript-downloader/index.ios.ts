import { NativescriptDownloaderCommon, DownloadOptions, DownloadEventData, StatusCode, ProgressEventData, generateId } from './common';

import { path as nsPath, knownFolders } from '@nativescript/core';
		...
		const task = download.downloadTaskWithRequestProgressDestinationCompletionHandler(
			...
			(response, filePath, error) => {
				...
				if (error) {
					...
					const state = this.tasksReader.readProp(task, 'state', interop.types.int32);
					if (state === NSURLSessionTaskState.Completed && !task.error) {
						if (owner.downloadsData.has(id)) {
							const data = owner.downloadsData.get(id);
